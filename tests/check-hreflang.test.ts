...
function healthyPair(): Record<string, string> {
  return { en: EN_URL, "es-CR": ES_URL, "x-default": EN_URL };
}
...
describe("auditDist (I18N-04)", () => {
  ...
  it("fails when x-default is missing", async () => {
    const noDefault = { en: EN_URL, "es-CR": ES_URL };
    const problems = await audit(
      siteWith({
  ...
  it("fails when the pair is asymmetric", async () => {
    const problems = await audit(
      siteWith({
        "es/index.html": page({
          alternates: { en: EN_URL, "es-CR": ES_URL, "x-default": ES_URL },
          canonical: ES_URL,
          lang: "es-CR",
        }),
      })
  ...
  it("fails when the hreflang labels are swapped", async () => {
    const swapped = { en: ES_URL, "es-CR": EN_URL, "x-default": ES_URL };
    const problems = await audit(
      siteWith({
  ...
  it("fails when a page does not reference itself", async () => {
    ...
    const notSelf = { en: otherEn, "es-CR": ES_URL, "x-default": otherEn };
    const problems = await audit(
      siteWith({
  ...
  it("fails when a locale ships no pages at all — both or neither", async () => {
    const enOnly = { en: EN_URL, "es-CR": ES_URL, "x-default": EN_URL };
