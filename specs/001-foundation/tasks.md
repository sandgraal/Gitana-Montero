...
- [ ] **T104 [PLATFORM]** Base entry schemas in `src/content.config.ts`: shared `data` + locale-keyed `prose` (both required), `fitment` placeholder type, `confidence` enum, `sources` array with archiveUrl. Per-locale slug registry. Activates T103 graders. Depends: T103 merged. *(I18N-05, I18N-06, SCF-04)*
