...
- [ ] **T104 [PLATFORM]** Base entry schemas: implement the T103 seam — pure-Zod building blocks in `src/schemas/entry.ts` + `src/schemas/slugs.ts` (stub signatures and grader-enforced contract are binding), registered via `defineCollection` in `src/content.config.ts` — shared `data` + locale-keyed `prose` (both required), `fitment` placeholder type, `confidence` enum (order per spec §2, ratified 2026-08-27), `sources` array with archiveUrl. Per-locale slug registry. Activates T103 graders; update/delete `src/scaffold.smoke.test.ts` (asserts empty collections). Depends: T103 merged. *(I18N-05, I18N-06, SCF-01, SCF-04)*
