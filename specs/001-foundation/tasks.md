...
- [ ] **T207 [CONTENT]** Reference data: FSM section index (citations only), fluid chart, torque master table, capacities/dimensions — fitment-scoped, every value cited. Note (T106 review): the schema change adding this collection's numeric fields is the FIRST live exercise of check:citations on real content (until then only scratch fixtures have reached it) — the paired schema work must demonstrate the check firing on its fields. SOURCE_KINDS `manufacturer`/`reference` were pulled forward (plan.md amended 2026-08-28; fix/001-source-kinds). T207's schema work must still decide the kind→tier coherence rule (nothing today stops an `fsm-confirmed` entry citing only a forum source — T207 designs the mapping or explicitly rules it a review-time concern). Depends: T203. *(REF-01, REF-02)*
